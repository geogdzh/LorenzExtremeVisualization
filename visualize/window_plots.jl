...
function plot_evolution(n, k, ref_list, static_ref; sliding_windows=nothing, middle_values=nothing, bayesian_delta=nothing, ensemble_generators=nothing)
    ...
    scatter!(ref_list, entry_list, color=:black, label="static ρ ref.")
    
    if !isnothing(sliding_windows)
    ...
    if !isnothing(bayesian_delta) 
        ...
        deltamean, deltastd = get_means_stds([bayesian_delta], n, k) #returns two lists of length 1
        scatter!([29], deltamean, color="blue", label="changing ρ avg.")
    ...
    if !isnothing(ensemble_generators)
        ...
        band!(middle_values, ensemble_means .- ensemble_stds .* 2, ensemble_means .+ ensemble_stds .* 2, color=(:blue, 0.3) , label="ensemble 2σ")
    end
